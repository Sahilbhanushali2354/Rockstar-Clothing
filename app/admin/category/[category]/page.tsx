...
import { Modal, Button, Input, Form, message, Table, Upload, Image, Row, Col, Card, Select, Spin } from 'antd';
...
const CategoryPage = () => {
  ...
  const handleUpdate = async (values: any) => {
    ...
    try {
      setLoading(true);
      let updatedFileUrl = selectedItem.file?.url; // Existing file URL, if any

      // Check if a new file has been uploaded (instance of File)
      ...
      // Update Firestore document with updated data (including the file URL)
      const docRef = doc(FStore, category as string, selectedItem.id);
      await updateDoc(docRef, {
        ...values,
        file: { url: updatedFileUrl }, // Store the file URL in Firestore
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setDeleteItemId(null); setLoading(false)
    } catch (error) {
      setLoading(false)
  ...
  const handleImageChange = (file: any) => {
    if (file.fileList.length) {
      const newFile = file.fileList[0].originFileObj;
      setSelectedItem((prev: any) => ({ ...prev, file: newFile }));
    }
  };
                ...
                <Form.Item label={<span className="text-gray-300">Product Image</span>}>
                  ...
                  {selectedItem.file && (
                    <Image
                      ...
                      width={40}
                      src={selectedItem.file?.url || URL.createObjectURL(selectedItem.file)}
